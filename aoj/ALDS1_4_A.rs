...
fn main() {
    ...
    let mut s: Vec<Element> = read_line_into_vec(&mut reader);
    let q: Element = read_line(&mut reader);
    let t: Vec<Element> = read_line_into_vec(&mut reader);
    println!(
        "{}",
        t.iter()
            .filter_map(|key| linear_search(&mut s, *key))
            .count()
    );
}

fn linear_search(list: &mut Vec<i32>, v: i32) -> Option<usize> {
    list.push(v);
    let mut i = 0;
    while list[i] != v {
        i = i + 1;
